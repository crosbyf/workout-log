  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
      ...
      >
        ...
        >
          <h2 className={`text-xl font-bold ${currentTheme.text}`}>Backups</h2>
          <button
            onClick={handleClose}
        ...
        <div className="px-6 py-6">
          {loading ? (
            ...
            <div className="space-y-3">
              {backups.map((backup, index) => (
                <div
                  key={index}
                  className={`p-4 rounded-lg border ${currentTheme.cardBorder} flex items-start justify-between`}
                >
                  <div className="flex-1">
                    <p className={`font-semibold ${currentTheme.text}`}>
                      {formatDate(backup.timestamp)}
                    </p>
                    <div className={`text-sm ${currentTheme.text} opacity-70 mt-2 space-y-1`}>
                  ...
                  <button
                    ...
                    className="ml-4 px-4 py-2 rounded-lg bg-green-500/20 border border-green-500/50 text-green-400 hover:bg-green-500/30 transition-colors flex items-center gap-2 font-medium whitespace-nowrap"
                  >
                    <Download size={18} />
                    Restore
                  </button>
          ...
          {/* Info */}
          <div className={`mt-6 p-4 rounded-lg ${currentTheme.inputBg} text-sm ${currentTheme.text} opacity-75`} style={{ backgroundColor: currentTheme.rawInputBg, color: currentTheme.rawText }}>
