...
export default function WeightTracker() {
          ...
          return (
            <g key={i}>
              <line
                ...
                x2={width - padding}
                y2={y}
                stroke={currentTheme.rawCardBorder}
                strokeWidth="1"
                strokeDasharray="4"
                opacity="0.5"
              />
  ...
  return (
    <div className={`pb-20 ${currentTheme.bg}`} style={{ backgroundColor: currentTheme.rawBg }}>
      ...
      {/* Add Weight Button - Full width */}
      <button
        onClick={handleAddWeight}
        className="w-full mx-4 mb-6 px-4 py-3 rounded-lg bg-blue-500 text-white font-semibold hover:bg-blue-600 transition-all"
      >
      ...
      {/* Summary Card */}
      {summaryStats && (
        <div className={`rounded-xl p-5 mb-6 ${currentTheme.cardBg} mx-4`} style={{ backgroundColor: currentTheme.rawCardBg }}>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <div className={`text-xs uppercase tracking-wider opacity-50 ${currentTheme.text}`}>CURRENT</div>
              <div className={`text-3xl font-bold ${currentTheme.text}`}>{summaryStats.currentWeight} lbs</div>
            </div>
            <div>
              <div className={`text-xs uppercase tracking-wider opacity-50 ${currentTheme.text}`}>STARTING</div>
              <div className={`text-3xl font-bold ${currentTheme.text}`}>{summaryStats.startingWeight} lbs</div>
            </div>
            <div>
              <div className={`text-xs uppercase tracking-wider opacity-50 ${currentTheme.text}`}>TOTAL CHANGE</div>
              <div className={`text-3xl font-bold ${summaryStats.change >= 0 ? 'text-red-500' : 'text-green-500'}`}>
                {summaryStats.change >= 0 ? '+' : ''}{summaryStats.change.toFixed(1)} lbs
              </div>
            </div>
            <div>
              <div className={`text-xs uppercase tracking-wider opacity-50 ${currentTheme.text}`}>RATE</div>
              <div className={`text-3xl font-bold ${currentTheme.text}`}>{summaryStats.ratePerWeek} lbs/week</div>
            </div>
          </div>
          <div className={`text-xs opacity-50 mt-4 pt-4 border-t ${currentTheme.text}`} style={{ borderColor: currentTheme.rawCardBorder }}>
      ...
      {chartEntries.length > 1 && (
        <div className={`rounded-xl p-5 mb-6 ${currentTheme.cardBg} mx-4`} style={{ backgroundColor: currentTheme.rawCardBg }}>
          <h3 className={`text-lg font-bold mb-4 ${currentTheme.text}`}>Weight Trend Chart</h3>
          {renderChart()}
        </div>
      )}
