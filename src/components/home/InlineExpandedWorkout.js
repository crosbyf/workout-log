...
import { Copy, Share2, Pencil, Trash2, Check, Clock, ChevronUp, Coffee, Footprints } from 'lucide-react';
    ...
    return (
      ...
      >
        <button
          ...
          style={{ outline: 'none' }}
        >
          <Footprints size={14} style={{ color: '#f59e0b' }} className="shrink-0 mr-2" />
          <span className="text-sm font-semibold" style={{ color: 'var(--color-text)' }}>
            {formatDate(workout.date)}
          </span>
          <span className="text-xs ml-1.5" style={{ color: 'var(--color-text-dim)' }}>· Run</span>
          <ChevronUp size={14} style={{ color: 'var(--color-text-dim)' }} className="ml-auto" />
        </button>
