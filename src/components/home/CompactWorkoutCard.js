...
export default function CompactWorkoutCard({ workout, isExpanded, onToggle, presets = [], proteinGrams = 0 }) {
  ...
  if (workout.isRun) {
    return (
      <button
        onClick={onToggle}
        className="w-full text-left rounded-lg overflow-hidden mb-1.5 flex items-center"
        style={{
      ...
      >
        ...
        <div className="flex-1 flex items-center px-3 py-3 min-w-0 gap-3">
  ...
  if (workout.isDayOff) {
    return (
      <button
        onClick={onToggle}
        className="w-full text-left rounded-lg mb-1.5 px-3 py-2.5 flex items-center gap-2"
        style={{
          backgroundColor: 'var(--color-surface)',
