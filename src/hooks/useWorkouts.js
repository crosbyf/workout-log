...
function clearDeletedIds() {
  ...
  upsertSetting(DELETED_IDS_SETTING_KEY, JSON.stringify([]));
}
...
export function useWorkouts() {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchWorkouts(), fetchAndMergeDeletedIds()]).then(([remote, deletedIds]) => {
      ...
      // Only remove from tracking after confirmed deletion
      Promise.all(
        [...deletedIds].map(id =>
          deleteWorkoutRemote(id).then(ok => ok ? id : null)
        )
      ).then(results => {
        const succeeded = results.filter(Boolean);
        if (succeeded.length > 0) {
          // Remove only successfully deleted IDs
          const current = getDeletedIds();
          for (const id of succeeded) current.delete(id);
          const arr = [...current];
          setItem(DELETED_IDS_KEY, arr);
          upsertSetting(DELETED_IDS_SETTING_KEY, JSON.stringify(arr));
        }
